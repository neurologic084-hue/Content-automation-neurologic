...
//      reverb removal + adaptive leveler. Its free tier prepends a spoken ad,
//      which auphonicCleanInPlace trims off by the exact video/audio length diff.
...
export async function cleanAudioInPlace(videoPath: string): Promise<AudioCleaner> {
  ...
  if (process.env.SUBMAGIC_API_KEY) {
    try {
      ...
      console.warn('[audio-clean] Submagic clean-audio failed/over-limit, trying Auphonic:', (e as Error).message)
    }
  } else {
    console.warn('[audio-clean] SUBMAGIC_API_KEY not set — trying Auphonic')
  }

  // 2. Auphonic (backup — only reached when Submagic fails or is over its limit).
  ...
  if (auth) {
    try {
      ...
      console.warn('[audio-clean] Auphonic clean-audio failed, falling back to ElevenLabs isolation:', (e as Error).message)
    }
  }

  // 3. ElevenLabs isolation (final fallback). 4. Original audio if that fails too.
