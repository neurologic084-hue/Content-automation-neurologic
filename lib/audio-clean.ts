...
//
// (Auphonic used to sit between Submagic and ElevenLabs as backup #1; removed
// 2026-07 — the account was out of credits so it only added a failed round-trip
// to every fallback, and Submagic is the only cleaner we intend to pay for.)
//
...
export async function cleanAudioInPlace(videoPath: string): Promise<AudioCleaner> {
  ...
  // 2. ElevenLabs isolation (fallback). 3. Original audio if that fails too.
