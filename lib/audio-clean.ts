...
//
//   1. Auphonic (auphonic.com) when AUPHONIC_API_TOKEN (or username/password)
...
export type AudioCleaner = 'auphonic' | 'elevenlabs' | 'none'
...
export async function cleanAudioInPlace(videoPath: string): Promise<AudioCleaner> {
  ...
  if (auth) {
    ...
    console.log('[audio-clean] AUPHONIC_API_TOKEN not set — using ElevenLabs isolation')
  }
