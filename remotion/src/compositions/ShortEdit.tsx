...
const DesignedCover: React.FC<{
  ...
  return (
    <AbsoluteFill>
      ...
      <div style={{ position: 'absolute', left: hasMedia ? '7%' : '10%', top: hasMedia ? '31%' : '38%', width: hasMedia ? '62%' : '80%', textAlign: hasMedia ? 'left' : 'center' }}>
        <div
          style={{
            position: 'absolute',
            inset: '-16% -10%',
        ...
        <div
          style={{
            ...
            fontSize: widthPx * (hasMedia ? 0.082 : 0.096),
            lineHeight: 1.04,
        ...
        >
          {words.map((w, i) => {
            ...
            return (
              <span key={i} style={{ display: 'inline-block', opacity: o, translate: `0px ${rise}px`, paddingRight: '0.24em' }}>
                {w}
              </span>
            )
          })}
...
const ViralCaptionPage: React.FC<{ page: ShortEditPage; widthPx: number }> = ({ page, widthPx }) => {
  ...
  const renderWords = (words: string[], style: React.CSSProperties, pop: boolean) =>
    words.map((w, i) => {
      ...
      return (
        <span key={i} style={{ display: 'inline-block', ...style, opacity: anim.opacity, scale: String(anim.scale), padding: '0 0.09em' }}>
          {w}
        </span>
      )
    })
